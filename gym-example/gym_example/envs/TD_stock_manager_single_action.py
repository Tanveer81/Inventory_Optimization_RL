...
class StockManagerSingleAction(gym.Env, ABC):
    ...
    def reward_for_one_inner_timestep(self, mat_i):
        ...
        reward = self.stock_out_costs[mat_i] * min(0, self.stock_level[mat_i]) \
                 - self.inv_costs[mat_i] * max(0, self.stock_level[mat_i])
    ...
    def update_stock_level_and_reward(self, actions):
        ...
        for i in range(len(self.stock_level)):
            ...
            for j in range(self.delivery_time[i]):
                ...
                self.stock_level[i] = self.stock_level[i] - self.history.iloc[j + self.monitor_timestep[i], i]
                rewards.append(self.reward_for_one_inner_timestep(i))

            self.monitor_timestep[i] += self.delivery_time[i]
    ...
    def reset(self):
        ...
        if self.random_reset:
            # initialize an empty Demand History (demand_history)
            self.monitor_timestep = [random.randint(self.past_demand, self.delivery_time[i] + self.past_demand)
